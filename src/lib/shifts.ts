...
export function getShiftTemplate(
  ...
  /** Für die Pausenlänge (Minijob 30 statt 60). Ohne = Stammkraft-Regel. */
  employmentType?: EmploymentType,
): ShiftTemplate {
  const paidMinutes = paidHours * 60;
  const pauseMinutes = calculatePause(paidMinutes, employmentType);
  const presence = presenceFromPaid(paidMinutes, employmentType);
