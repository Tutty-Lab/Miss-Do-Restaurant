...
it("preserves legacy shifts, employee targets and print locks on load", () => {
  const old: Schedule = {
    ...
    sundayCleaningMinutes: undefined, lockedAt: "2026-09-01T12:00:00Z", printedWeeks: ["2026-08-03"],
  ...
  old.workHours.perWeekday.friday[0].endMinutes = 1200;
  ...
  expect(loaded.workHours.perWeekday.friday[0].endMinutes).toBe(1200);
  expect(validateSchedule(loaded.employees, loaded.shifts, false).errors).toEqual([]);
  expect(workHoursForGeneration(loaded).perWeekday.friday[0].endMinutes).toBe(1320);
...
it("retains custom work hours and configured surcharge settings after reload", () => {
  ...
  schedule.sundayCleaningMinutes = 0;
  ...
  expect(loaded.sundayCleaningMinutes).toBe(0);
